...
#load dataset
data2 <- read.csv("diamonds.csv", header = TRUE, sep = ",")
attach(data2)
...
summary(result_trans_full)
anova(result_trans_full)

#check regression assumptions
#residual plot
plot(result_trans_full$fitted.values, result_trans_full$residuals, main = "Residual Plot (Full Transformed Price)")

#check if we should transform carat
plot(result_trans_full2$fitted.values, result_trans_full2$residuals, main = "Residual Plot (Full Transformed Price)")

#ACF plot
acf(result_trans_full$residuals, main ="ACF Plot with Transformed Price for Full Model")

##QQ plot of residuals for FULL
qqnorm(result_trans_full$residuals)
qqline(result_trans_full$residuals)

sum(anova(result_trans_full)[2])
